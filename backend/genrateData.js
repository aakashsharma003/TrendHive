  ...
  try {
    ...
    for (let i = 0; i < 150; i++) {
      const product = new Product({
        ...
        stock_quantity: faker.datatype.number({ min: 1, max: 100 }),
    ...
    for (let i = 0; i < 30; i++) {
      const order = new Order({
        ...
        total_amount: faker.datatype.float({ min: 20, max: 1000 }),
    ...
    for (let i = 0; i < 50; i++) {
      const wishlistItem = new Wishlist_Item({
        wishlist_id: wishlists[Math.floor(Math.random() * wishlists.length)]._id,
        product_id: products[Math.floor(Math.random() * products.length)]._id,
      });
    ...
    for (let i = 0; i < 30; i++) {
      const payment = new Payment({
        ...
        amount: faker.datatype.float({ min: 20, max: 1000 }),
        status: "Paid",
      });
